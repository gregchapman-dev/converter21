...
class HumdrumFileTestResults:
    ''' HumdrumFileTestResults holds the original Humdrum test file contents, the original results file contents,
        ...
        self.fileContents: list[str] = the original Humdrum test file contents
        self.results: dict = the original results file contents, as follows:
            self.results["exclusiveInterpretationlineNumbers"]: list[int] = list of line nums of the '**' lines
            self.results["manipulatorLineNumbers"]: list[int] = list of line nums of '*^', '*v', etc lines
            self.results["tokenDataTypeChanges"]: {} = dict(lineNumberOfChange: str(int), newTokenDataTypes: list[str])
...
#             outResultsFile.write(json.dumps(self.results, indent=4))
    ...
    def generateExpectedMaxTrack(self, tokens: list[list[str]], initialTrackCount: int) -> int:
    ...
    def generateExpectedLines(self, entireHumdrumFileString: str) -> list[str]:
        return entireHumdrumFileString.split('\n')

    def generateExpectedTokens(self, allLines: list[str]) -> list[list[str]]:
    ...
    def splitIntoTokens(self, line: str) -> list[str]:
        output: list[str] = []
    ...
    def generateExpectedLineTypes(self, allLines: list[str]) -> list[str]:
        lineTypes = []
        for line in allLines:
    ...
    def generateExpectedTokenStrings(self, stringListChanges: dict, lineCount: int) -> list[list[str]]:
    ...
    def generateExpectedIs__FromTrueLineNumbers(self, trueLineNumbers: list[int], numValues: int) -> list[bool]:
        expectedIs__: list[bool] = [False] * numValues
...
def getTokenDataTypes(hf: HumdrumFile) -> list[list[str]]:
