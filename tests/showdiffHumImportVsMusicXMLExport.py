...
def runTheFullTest(inputPath: Path):
    print(f'Parsing Humdrum file: {inputPath}')
    score1 = m21.converter.parse(inputPath, format='humdrum', forceSource=True)
    ...
    print('comparing the two m21 (Humdrum and MusicXML) scores')
    score_lin2 = AnnScore(
        score1, DetailLevel.AllObjects | DetailLevel.Style | DetailLevel.Metadata
    )
    print('loaded imported Humdrum score')
    score_lin3 = AnnScore(
...
#     print('imported Humdrum score written to: ', score1.write('musicxml', makeNotation=False))
