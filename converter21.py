...
from music21 import converter, stream, exceptions21
#from humdrum import BetterHumdrumConverter
    ...
    if whichList == 'input':
        ...
        for subc in inList:
            if subc.registerInputExtensions:
                print('\tformats   : ' + ', '.join(subc.registerFormats) + '\textensions: ' + ', '.join(subc.registerInputExtensions))
    else:
        outList = c.subconvertersList('output')
        print("Supported output formats are:")
        for subc in outList:
            if subc.registerOutputExtensions:
                print('\tformats   : ' + ', '.join(subc.registerFormats) + '\textensions: ' + ', '.join(subc.registerOutputExtensions))
...
#converter.registerSubconverter(BetterHumdrumConverter)

parser = argparse.ArgumentParser()
parser.add_argument("inputFile",
                    help="input music file to convert from (extension is used to determine input format if -if/--inputFormat is not specified")
parser.add_argument("outputFile",
                    help="output music file to convert to (extension is NOT used to determine/validate output format, so if you're not careful you'll end up with contents not matching extension)")
parser.add_argument("-if", "--inputFormat",
                    choices=getInputFormatsList(),
                    help="format of the input file (only necessary if inputFile has no supported extension)")
parser.add_argument("-of", "--outputFormat", required=True,
                    choices=getOutputFormatsList(),
                    help="format of the output file (required)")
parser.add_argument("-fs", "--forceSource", type=bool, default=False,
...
if (args.inputFormat is None):
    ...
    if fileExt not in getInputExtensionsList():
        print("Input file extension '{}' not supported unless inputFormat specified.".format(fileExt))
...
#s.show()
#s.show('text')
s.write(fmt=args.outputFormat, fp=args.outputFile)
...
# Validate outputFile extension by hand, and if necessary change it to be valid for the output format.
...
if outFileExt not in validOutputExtList:
    # because we already checked the outputFormat, we can assume that there is a valid extension for it
